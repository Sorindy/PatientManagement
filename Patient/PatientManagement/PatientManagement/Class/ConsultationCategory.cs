...
namespace PatientManagement.Class
{
    ...
    public  class ConsultationCategory : ICategory
    {
        ...
        public void Delete(int id)
        {
            ...
            _db.ConsultationCategories.AddOrUpdate(delete);
            _db.SaveChanges();
        }

        public object Show()
        {
            var getcategory = from v in _db.ConsultationCategories where v.Available
                select new
                {
                    v.Id,v.Name
                };
        ...
        {
            var getcategory = _db.ConsultationCategories.Where(v=>v.Available);
        ...
        {
            var getcategory = _db.ConsultationCategories.Where(v=>v.Available).Single(v => v.Id  == categoryId );
            return getcategory .Id;
        }

        public Dictionary<int, string> ShowCategoryForDoctor(int workerId)
        {
            var getcategory = _db.Managements.First(v => v.Account.WorkerId == workerId).ConsultationCategories.Where(v=>v.Available);
            var dic = new Dictionary<int, string>();
            foreach (var item in getcategory)
            {
                dic.Add(item.Id, item.Name);
