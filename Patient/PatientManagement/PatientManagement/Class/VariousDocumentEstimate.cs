   ...
   public class VariousDocumentEstimate: IEstimate 
    {
        ...
        private void CreateVisit(int patientId, string description)
        {
            try
            {
                ...
                _db.Visits.First(v => v.Id == getVisit.Id && v.VisitCount == getVisit.VisitCount).VariousDocumentEstimates.Add(get);
                _db.SaveChanges();
            }
            catch
            {
        ...
        {
            var get = _db.VariousDocumentEstimates.First(v => v.Id == estimateId).ReferrerId;
            return get.ToString();
        }

        public string GetNurseId(int estimateId)
        {
            var get = _db.VariousDocumentEstimates.First(v => v.Id == estimateId).NurseId;
            return get.ToString();
        }
    }
}
