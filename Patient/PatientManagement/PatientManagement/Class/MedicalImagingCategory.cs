...
namespace PatientManagement.Class
{
    public class MedicalImagingCategory : ICategory
    {
        ...
        public Hospital_Entity_Framework.WaitingList CheckWaitingList(int patientId, int categoryId)
        {
            ...
            var get = firstOrDefault != null && firstOrDefault.MedicalImagingCategories.Any(v => v.Id == categoryId);
            if (get)
            {
        ...
        public void Delete(int id)
        {
            ...
            _db.MedicalImagingCategories.AddOrUpdate(delete);
            _db.SaveChanges();
        }
        ...
        {
            var getcategory = _db.MedicalImagingCategories.Where(v => v.Available);
        ...
        {
            var getcategory = _db.MedicalImagingCategories.Where(v => v.Available).Single(v => v.Id == categoryId);
            return getcategory.Id;
        }

        public Dictionary<int, string> ShowCategoryForDoctor(int workerId)
        {
            var getcategory = _db.Managements.First(v => v.Account.WorkerId == workerId).MedicalImagingCategories.Where(v => v.Available);
            var dic = new Dictionary<int, string>();
            foreach (var item in getcategory)
            {
                dic.Add(item.Id, item.Name);
